...
const HomeFoodDetail = ({ route }) => {
  ...
  const { likedPosts, savedPosts, unlikedPosts } = useSelector(
    (state) => state.posts
  );
  ...
  const handleUnlike = () => {
    ...
    // if (!isUnliked) {
    //   dispatch(unlikePost(recipeId));
    //   if (likedPosts.includes(recipeId)) {
    //     dispatch(likePost(recipeId)); // Remove from likedPosts if it was liked before
    //   }
    // }
  };
